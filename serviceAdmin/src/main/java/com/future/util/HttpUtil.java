...
public class HttpUtil {
     ...
     * @throws IOException
     */
    public static void downloadFile(String url,String path,String fileUrl) throws IOException {
        log.debug("path:" + path);
        log.debug("url:" + url);
        HttpClient client = null;
        File f = null;
            ...
            if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                ...
                try {
                    ...
                    try {
                        if (bw != null)
                            bw.close();
                        //com.cmcc.andedu.util.HttpClient.upload(f, "http://112.33.2.95:9000/educloud/study/mic" + fileUrl.substring(0,fileUrl.lastIndexOf("/")));
                        f.delete();
                    } catch (Exception e) {
                        log.error(
